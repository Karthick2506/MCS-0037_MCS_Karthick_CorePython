...
It is intended as a set of tools that all work together: text editor, compiler, build or make integration, debugging, etc.
Virtually all IDEs are tied specifically to a language or framework or tightly collected set of languages or frameworks.

An editor is simply that, a tool that is designed to edit text.
Typically they are optimized for programming languages though many programmer's text editors are branching out and adding
features for non-programming text.
The key here is that text editors are designed to work with whatever language or framework you choose.
Code editors are text editors with powerful built-in features and specialized functionalities to simplify and accelerate
...
It comes in two versions community version and a professional version where community version is free but the professional
version is paid.
...
6.	Syntax and error maintenance, it can help in the analysation of syntax error even before you compile your code reduce
any overhead.
7.	Language injections, it allows you to use the output of a function, the value of variables or any object inside a
template string that can be injected inside an HTML tag for further usage.
8.	Import assistance, this can allow importing missing libraries from another part of the project.
9.	Code completion, allows you to complete name of classes, methods, parameters and other variables depending upon the context.
...
Spyder:
-------
Spyder is another good open-source and cross-platform IDE written in Python. It is also called Scientific Python Development IDE
and it is the most lightweight IDE for Python. It is mainly used by data scientists who can integrate with Matplotlib, SciPy, NumPy,
Pandas, Cython, IPython, SymPy, and other open-source software. It comes with the Anaconda package manager distribution
and it has some good advanced features such as edit, debug, and data exploration.
